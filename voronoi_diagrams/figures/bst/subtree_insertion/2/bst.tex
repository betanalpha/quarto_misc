...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 0)}]
    ...
    \node (N6) at (-2 * \d, -2 * \d) [unobsell] { $n_{2}, n_{9}$ };
    \node (N7) at (-3 * \d, -3 * \d) [unobscirc] { $n_{2}$ };
    \node (N8) at (-1 * \d, -3 * \d) [unobsell] { $n_{9}, n_{2}$ };
    \node (N9) at (-1.5 * \d, -4 * \d) [unobscirc] { $n_{9}$ };
    \node (N10) at (-0.5 * \d, -4 * \d) [unobscirc] { $n_{2}$ };

    \foreach \B/\E in {N2/N6, N6/N7, N6/N8, N8/N9, N8/N10} {
      \draw[->, >=stealth, color=mid, line width=1.5] (\B) -- (\E);
    }
